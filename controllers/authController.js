...
const loginUser = async (request, response) => {
  try {
    ...
    const accessToken = jwt.sign({ id: user._id, diabetes: user.diabetes }, process.env.ACCESS_TOKEN_KEY, {
      expiresIn: '1h'
    });

    //En una cookie de tipo httpOnly
    const refreshToken = jwt.sign({ id: user._id, diabetes: user.diabetes }, process.env.REFRESH_TOKEN_KEY, {
      expiresIn: '1d'
    });
